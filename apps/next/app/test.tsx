...
export default function Test() {
  ...
  return !address ? (
    ...
    <div className="flex flex-col gap-2">
      <p className="text-center text-sm text-white">{address}</p>
      <div className="flex justify-center">
        <button className="p-2 bg-primary rounded-md" onClick={() => approveWithModal()}>
          Approve
        </button>
      </div>
      <div className="flex justify-center">
        <button className="p-2 bg-primary rounded-md" onClick={() => disconnect()}>
          Disconnect
        </button>
      </div>
    </div>
